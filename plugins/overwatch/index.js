...
function getRegionStats (battletag, givenRegion, allStats) {
  ...
  if (givenRegion && allStats[givenRegion]) {
    return { region: givenRegion, stats: allStats[givenRegion].stats, heroes: allStats[givenRegion].heroes }
  }
...
function generateDiscordEmbed (battletag, region, competitiveStats, competitiveHeroPlaytime, client) {
  const c = competitiveStats

  let embed = new Discord.RichEmbed()

  embed.setAuthor(`${battletag} (${region.toUpperCase()})`, getRankIcon(c.overall_stats.comprank) || c.overall_stats.avatar, `https://playoverwatch.com/en-gb/career/pc/${region}/${battletag.replace('#', '-')}`)

  embed.setDescription(`${c.overall_stats.games} ${c.competitive ? 'competitive' : 'quickplay'} games played (${c.overall_stats.win_rate}% won) over ${c.game_stats.time_played} hours.`)
  ...
  embed.addField('Most Played Hero', getMostPlayedHero(competitiveHeroPlaytime), true)
  ...
  embed.addField('Environmental', `**Kills**: ${c.game_stats.environmental_kills || 'n/a'}\n**Deaths**: ${c.game_stats.environmental_deaths || 'n/a'}`, true)
  ...
  embed.addField('Offensive Assits', `**Average**: ${c.average_stats.offensive_assists_avg || 'n/a'}\n**Most**: ${c.game_stats.offensive_assists_most_in_game}\n**Total**: ${c.game_stats.offensive_assists}`, true)

  embed.addField('Defensive Assists', `**Average**: ${c.average_stats.defensive_assists_avg || 'n/a'}\n**Most**: ${c.game_stats.defensive_assists_most_in_game}\n**Total**: ${c.game_stats.defensive_assists}`, true)

  embed.addField('Objective Time', `**Average**: ${formatDuration(c.average_stats.objective_time_avg)}\n**Most**: ${formatDuration(c.game_stats.objective_time_most_in_game)}\n**Total**: ${formatDuration(c.game_stats.objective_time)}`, true)

  embed.addField('Time on Fire', `**Average**: ${formatDuration(c.average_stats.time_spent_on_fire_avg)}\n**Most**: ${formatDuration(c.game_stats.time_spent_on_fire_most_in_game)}\n**Total**: ${formatDuration(c.game_stats.time_spent_on_fire)}`, true)
...
module.exports = (bot, config, client) => {
  bot.command(/^!(ow|overwatch) ([^0-9\s](\S{2,11})#(\d{4,}))( (eu|us|kr))?$/i, async (message, match) => {
    ...
    const { region, stats, heroes, error } = getRegionStats(battletag, givenRegion, allStats)
    if (error) return message.channel.send(error)

    const embed = generateDiscordEmbed(battletag, region, stats.competitive, heroes.playtime.competitive, client)
