...
module.exports = (bot, config, client) => {
  bot.command({
    regex: /^!trivia start$/i,
    usage: '!trivia start',
    description: 'starts a trivia game',
    function: (message, match) => {
      if (games[message.channel.id])
        return message.channel.send('Trivia game already in progress')

      games[message.channel.id] = startGame(
        message.channel.id,
        default_trivia,
        2
      )
  ...
  bot.command({
    ...
    description: 'attempt to answer the trivia question',
    function: (message, match) => {
      if (!games[message.channel.id])
        return message.channel.send('No trivia game is in progress')
      if (!games[message.channel.id].currentQuestion)
        return message.channel.send(`Too slow ${message.author.username}!`)
      ...
      if (attempt.toLowerCase() !== answer.toLowerCase())
        return message.channel.send(
          `*__${attempt}__* is not correct ${message.author.username}!`
        )

      game.currentQuestion = null
      game.points[message.author.username]
        ? game.points[message.author.username]++
        : (game.points[message.author.username] = 1)
      message.channel.send(
        `*__${attempt}__* is correct, 1 point for ${message.author.username}!`
      )
      ...
      if (
        game.pointsToWin &&
        game.points[message.author.username] === game.pointsToWin
      ) {
        return setTimeout(() => {
          message.channel.send(`${message.author.username} wins`)
          delete games[message.channel.id]
  ...
  bot.command({
    regex: /^!trivia scores$/i,
    usage: '!trivia scores',
    description: 'show the scores for the current trivia game',
    function: (message, match) => {
      if (!games[message.channel.id])
        return message.channel.send('No trivia game is in progress')
      ...
      var scores = Object.keys(points)
        .map(user => ({ user: user, value: points[user] }))
